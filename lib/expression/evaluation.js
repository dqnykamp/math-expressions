...
// Lightweight depth-first check for a specific operator in the AST.
// Used to skip expensive applyAllTransformations calls when the pattern can't match.
function treeContainsOperator(node, op) {
  if (!Array.isArray(node)) return false;
  if (node[0] === op) return true;
  for (let i = 1; i < node.length; i++) {
    if (treeContainsOperator(node[i], op)) return true;
  }
  return false;
}
...
const evaluate_to_constant = function (
  ...
  // Pre-check: patterns only apply when tree contains both "^" and "/" operators.
  // Skipping the traversal when impossible saves significant time for callers like
  // isNumerical() that invoke evaluate_to_constant for every candidate node.
  if (treeContainsOperator(tree, "^") && treeContainsOperator(tree, "/")) {
    var transformations = [];
