...
import {
  ...
  expandProfilerFinishRun,
  expandProfilerIncrementCounter,
  expandProfilerRecordTreeSize,
  expandProfilerStartRun,
...
function expand(expr_or_tree, no_division) {
  ...
  if (expandProfilerEnabled()) {
    ...
    expandProfilerIncrementCounter("expand.calls");
    expandProfilerRecordTreeSize("expand.input", tree);
  }

  let stepStart = expandProfilerStartTimer();
  tree = perform_matrix_multiplications(tree);
  expandProfilerEndTimer(stepStart, "expand.perform_matrix_multiplications");
  expandProfilerRecordTreeSize("expand.after_matrix_multiplications", tree);

  stepStart = expandProfilerStartTimer();
  tree = perform_vector_matrix_additions_scalar_multiplications(tree);
  expandProfilerEndTimer(
    stepStart,
    "expand.perform_vector_matrix_additions_scalar_multiplications",
  );
  expandProfilerRecordTreeSize("expand.after_vector_matrix_ops", tree);
  ...
  expandProfilerEndTimer(stepStart, "expand.applyAllTransformations");
  expandProfilerRecordTreeSize("expand.after_apply_all_transformations", tree);

  stepStart = expandProfilerStartTimer();
  tree = flatten.flatten(tree);
  expandProfilerEndTimer(stepStart, "expand.flatten");
  expandProfilerRecordTreeSize("expand.after_flatten", tree);

  stepStart = expandProfilerStartTimer();
  tree = evaluate_numbers(tree);
  expandProfilerEndTimer(stepStart, "expand.evaluate_numbers");
  expandProfilerRecordTreeSize("expand.after_evaluate_numbers", tree);

  stepStart = expandProfilerStartTimer();
  tree = collect_like_terms_factors(tree);
  expandProfilerEndTimer(stepStart, "expand.collect_like_terms_factors");
  expandProfilerRecordTreeSize("expand.after_collect_like_terms_factors", tree);

  stepStart = expandProfilerStartTimer();
  tree = normalize_negatives(tree);
  expandProfilerEndTimer(stepStart, "expand.normalize_negatives");
  expandProfilerRecordTreeSize("expand.output", tree);
