...
function collect_like_terms_factors(expr_or_tree, assumptions, max_digits) {
  ...
  function collectTreeFeatures(node, features) {
    ...
    if (node[0] === "apply" && typeof node[1] === "string") {
      features.applyFunctions[node[1]] = true;
    }
    if (node[0] === "unit") {
      features.hasUnitOperator = true;
    }

    for (let i = 1; i < node.length; i++) {
      collectTreeFeatures(node[i], features);
    }
  }
  ...
  let treeFeatures = {
    ...
    hasUnitOperator: false,
  };
  collectTreeFeatures(tree, treeFeatures);

  let applyFunctionsPresent = treeFeatures.applyFunctions;
  const hasUnitOperator = treeFeatures.hasUnitOperator;
  ...
  if (applyFunctionsPresent.nthroot) {
  ...
  if (
    applyFunctionsPresent.abs ||
    applyFunctionsPresent.sqrt ||
    applyFunctionsPresent.nthroot
  ) {
    // |x|^n -> x^n, where n is even
    transformations.push([
      ["^", ["apply", "abs", "x"], "n"],
